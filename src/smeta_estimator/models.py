...
class MachineLabourInfo:
    ...
    **Подтверждено устно на звонке со Smetrix (2026-09-18) и независимо —
    из первоисточника, публичной веб-страницы ФГИС ЦС (2026-09-24, см.
    CLAUDE.md, «Известные пробелы» → «Решено», «Трудозатраты машинистов»).**
    Практик подтвердил, и живая проверка страницы «Сметные цены на
    эксплуатацию машин и механизмов» на `fgiscs.minstroyrf.ru` показала то
    же самое: это не единая формула на все машины, а свойство конкретной
    позиции — для одной техники оплата труда машиниста уже учтена в её
    собственной цене (`labour_mach == 0`, обычно когда `driver_code` вообще
    не указан — самоходное/электрическое оборудование без отдельного
    оператора, на сайте «Оплата труда машинистов» = 0), для другой — не
    учтена и её нужно добавить отдельно (`labour_mach > 0`, на практике
    встречалось только `1.0`), умножив на текущую ставку машиниста по
    `driver_code` из того же `RimWorkerSalaryRegistry`, что уже используется
    для рабочих (`regional_pricing_client.fetch_worker_salary_registry`) —
    сайт публикует ровно эту сумму отдельным столбцом рядом с базовой ценой
    машины, не смешивая их.
    """
